    ...
    match tree {
        Tree::Node {
            ...
            if tree.height() == 1 {
                ...
                let hdist = hdist(tree.height() - 1) as i32;
...
fn root_xpos<T: fmt::Display>(tree: &Tree<T>, xpos: i32, height: usize) -> i32 {
    match tree {
        Tree::Node {
            value: _,
            left,
            ...
            Tree::Node { .. } => root_xpos(left, xpos, height - 1) + hdist(height) as i32,
            Tree::End => 1,
        },
        Tree::End => 0,
    }
